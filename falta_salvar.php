...
$query = mysql_query("SELECT Falta_Cod, Falta_Tent, ID, Nivel, Time FROM Usuarios WHERE ID_Cod = password('". $_COOKIE["usuarioid"] ."')");
...
$query = mysql_query("SELECT Rodada, Dinheiro_Falta, Convite, Status FROM Configuracoes");
$rs = mysql_fetch_array($query);

$rodada = $rs["Rodada"];
$dinheiro = $rs["Dinheiro_Falta"];
$convite = $rs["Convite"];
$status = $rs["Status"];
...
mysql_query("UPDATE Usuarios SET Falta = Falta + 1, Falta_Cod = '". $falta_cod ."', Falta_Tent = Falta_Tent - 1, Status = 1 WHERE ID_Cod = password('". $_COOKIE["usuarioid"] ."')");
...
if ($verificar_gol == 0) {
	...
	header("Location: falta_erro.php"); break;
}
?>