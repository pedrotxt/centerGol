...
$query = mysql_query("SELECT ID FROM Propostas WHERE Time = '". $mc_time ."' AND Usuario = '". $id ."'");
$rs = mysql_fetch_array($query);

if ($rs) {
	header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=2"); break;
}
...
if ($mc_time_dinheiro < $valor) {
	header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=3"); break;
}

$query = mysql_query("SELECT Time, Propostas, Trocas, Rescisao FROM Usuarios WHERE ID = '". $id ."'");
$rs = mysql_fetch_array($query);
...
if ($mc_time == $usuario_time) {
	header("Location: usuario_proposta.php?id=". $id); break;
}

if ($mc_time_dinheiro < $usuario_rescisao) {
	header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=4"); break;
}

if ($valor < $usuario_rescisao) {
	header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=5"); break;
}

if ($mc_vip_total <= 0 and $rescisao_dias_vip > 0 or $rescisao_dias_vip > $mc_vip_total and $rescisao_dias_vip > 0) {
	header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=6"); break;
}
...
if ($usuario_trocas >= $trocas_total) {
	header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=7"); break;
}

$data = date("d/m");

mysql_query("INSERT INTO Propostas (Time,Time_Atual,De,Usuario,Valor,Rescisao_Dias,Rescisao_Dias_VIP,Mensagem,Data) VALUES ('". $mc_time ."','". $usuario_time ."','". $mc_id ."','". $id ."','". $valor ."','". $rescisao_dias ."','". $rescisao_dias_vip ."','". $mensagem ."','". $data ."')");

header("Location: usuario_proposta.php?id=". $id ."&msg_proposta=1"); break;
?>