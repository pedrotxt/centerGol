...
$query = mysql_query("SELECT Count(ID) AS ranking_quantidade FROM Usuarios WHERE Gols_Rodada > 0");
...
if ($p > $p_total) {
	header("Location: ranking_rodada.php?p=1"); break;
}
...
<div class="box">
	<div class="topo_esquerdo"><div class="topo_direito"><div class="topo_meio"><h1>Ranking da Rodada</h1></div></div></div>
...
$query = mysql_query("SELECT Usuarios.ID as Usuario_ID, Usuarios.Usuario as Usuario_Nome, Usuarios.Nivel as Usuario_Nivel, Usuarios.Time as Usuario_Time, Usuarios.Gols_Rodada as Usuario_Gols_Rodada, Usuarios.VIP_Tempo as Usuario_VIP_Tempo, Usuarios.VIP_Cor as Usuario_VIP_Cor, Times.Time as Time_Nome FROM Usuarios INNER JOIN Times ON Times.ID = Usuarios.Time WHERE Gols_Rodada > 0 ORDER BY Gols_Rodada DESC LIMIT $inicio,$limite");
...
<?php } ?>
		...
		<td style="padding-top: 5px"><?=number_format($rs["Usuario_Gols_Rodada"],0,',','.')?></td>
	</tr>
