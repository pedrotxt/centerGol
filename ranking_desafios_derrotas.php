...
$query = mysql_query("SELECT Count(ID) AS ranking_quantidade FROM Usuarios WHERE Desafio_Derrotas > 0");
...
if ($p > $p_total) {
	header("Location: ranking_desafios_derrotas.php?p=1"); break;
}
...
<div class="box">
	<div class="topo_esquerdo"><div class="topo_direito"><div class="topo_meio"><h1>Ranking de Desafios de Derrotas</h1></div></div></div>
...
$query = mysql_query("SELECT Usuarios.ID as Usuario_ID, Usuarios.Usuario as Usuario_Nome, Usuarios.Nivel as Usuario_Nivel, Usuarios.Time as Usuario_Time, Usuarios.Desafio_Derrotas as Usuario_Desafio_Derrotas, Usuarios.VIP_Tempo as Usuario_VIP_Tempo, Usuarios.VIP_Cor as Usuario_VIP_Cor, Times.Time as Time_Nome FROM Usuarios INNER JOIN Times ON Times.ID = Usuarios.Time WHERE Desafio_Derrotas > 0 ORDER BY Desafio_Derrotas DESC LIMIT $inicio,$limite");
...
<?php if ($pos == 1) { ?>
	<tr height="25" bgcolor="#B6B6B6">
		...
		<td width="85" class="fonte1_negrito">Derrotas</td>
	</tr>
<?php } ?>
...
<?php } ?>
		...
		<td style="padding-top: 5px"><?=number_format($rs["Usuario_Desafio_Derrotas"],0,',','.')?></td>
	</tr>
...
if ($ranking_quantidade > $limite) {
	$p_nome = "ranking_desafios_derrotas";
