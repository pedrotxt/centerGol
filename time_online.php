...
$query = mysql_query("SELECT Count(ID) AS usuarios_online FROM Usuarios WHERE Time = '". $id ."' AND Status = 1");
...
if ($p > $p_total) {
	header("Location: time_online.php?id=". $id ."&p=1"); break;
}
...
<div class="box">
	<div class="topo_esquerdo"><div class="topo_direito"><div class="topo_meio"><h1>Usuários Online do <?=$time_nome?> (<?=$usuarios_online?>)</h1></div></div></div>
...
$query = mysql_query("SELECT Usuarios.ID as Usuario_ID, Usuarios.Usuario as Usuario_Nome, Usuarios.Nivel as Usuario_Nivel, Usuarios.Time as Usuario_Time, Usuarios.Gols_Total as Usuario_Gols_Total, Usuarios.VIP as Usuario_VIP, Usuarios.VIP_Cor as Usuario_VIP_Cor, Times.Time as Time_Nome FROM Usuarios INNER JOIN Times ON Times.ID = Usuarios.Time WHERE Usuarios.Time = '". $id ."' AND Status = 1 ORDER BY Ultimo_Acesso DESC LIMIT $inicio,$limite");
