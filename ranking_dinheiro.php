...
$query = mysql_query("SELECT Count(ID) AS ranking_quantidade FROM Usuarios WHERE Dinheiro > 0");
...
if ($p > $p_total) {
	header("Location: ranking_dinheiro.php?p=1"); break;
}
...
<div class="box">
	<div class="topo_esquerdo"><div class="topo_direito"><div class="topo_meio"><h1>Ranking de Dinheiro</h1></div></div></div>
...
$query = mysql_query("SELECT Usuarios.ID as Usuario_ID, Usuarios.Usuario as Usuario_Nome, Usuarios.Nivel as Usuario_Nivel, Usuarios.Time as Usuario_Time, Usuarios.Dinheiro as Usuario_Dinheiro, Usuarios.VIP_Tempo as Usuario_VIP_Tempo, Usuarios.VIP_Cor as Usuario_VIP_Cor, Times.Time as Time_Nome FROM Usuarios INNER JOIN Times ON Times.ID = Usuarios.Time WHERE Usuarios.Dinheiro > 0 ORDER BY Usuarios.Dinheiro DESC LIMIT $inicio,$limite");
...
<?php if ($pos == 1) { ?>
	<tr height="25" bgcolor="#B6B6B6">
		<td width="55">&nbsp;</td>
        <td width="200" style="padding-left: 31px" class="fonte1_negrito">Usuário</td>
		<td width="200" class="fonte1_negrito">Time</td>
		<td width="95" class="fonte1_negrito">Dinheiro</td>
	</tr>
<?php } ?>
...
<?php } ?>
		...
		<td style="padding-top: 5px"><?=number_format($rs["Usuario_Dinheiro"],0,',','.')?></td>
	</tr>
