...
$query = mysql_query("SELECT Count(ID) AS usuarios_secando FROM Usuarios WHERE Secar = '". $id ."'");
$rs = mysql_fetch_array($query);

$usuarios_secando = $rs["usuarios_secando"];

$p_total = ceil($usuarios_secando / $limite);
...
if ($p > $p_total) {
	header("Location: time_secando.php?id=". $id ."&p=1"); break;
}

$query = mysql_query("SELECT Time FROM Times WHERE ID = '". $id ."'");
$rs = mysql_fetch_array($query);
...
<div class="box">
	<div class="topo_esquerdo"><div class="topo_direito"><div class="topo_meio"><h1>Usuários Secando o <?=$time_nome?> (<?=$usuarios_secando?>)</h1></div></div></div>
	<div class="conteudo" style="padding-top: 4px">

<?php if ($usuarios_secando > 0) { ?>
...
$query = mysql_query("SELECT Usuarios.ID as Usuario_ID, Usuarios.Usuario as Usuario_Nome, Usuarios.Nivel as Usuario_Nivel, Usuarios.Time as Usuario_Time, Usuarios.Gols_Total as Usuario_Gols_Total, Usuarios.VIP as Usuario_VIP, Usuarios.VIP_Cor as Usuario_VIP_Cor, Times.Time as Time_Nome FROM Usuarios INNER JOIN Times ON Times.ID = Usuarios.Time WHERE Usuarios.Secar = '". $id ."' ORDER BY Ultimo_Acesso DESC LIMIT $inicio,$limite");
...
<?php if ($while == 1) { ?>
	<tr height="25" bgcolor="#B6B6B6">
        <td width="235" style="padding-left: 31px" class="fonte1_negrito">Usuário</td>
