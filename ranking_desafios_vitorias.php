...
$query = mysql_query("SELECT Count(ID) AS ranking_quantidade FROM Usuarios WHERE Desafio_Vitorias > 0");
...
if ($p > $p_total) {
	header("Location: ranking_desafios_vitorias.php?p=1"); break;
}
...
<div class="box">
	<div class="topo_esquerdo"><div class="topo_direito"><div class="topo_meio"><h1>Ranking de Desafios de Vitórias</h1></div></div></div>
...
$query = mysql_query("SELECT Usuarios.ID as Usuario_ID, Usuarios.Usuario as Usuario_Nome, Usuarios.Nivel as Usuario_Nivel, Usuarios.Time as Usuario_Time, Usuarios.Desafio_Vitorias as Usuario_Desafio_Vitorias, Usuarios.VIP_Tempo as Usuario_VIP_Tempo, Usuarios.VIP_Cor as Usuario_VIP_Cor, Times.Time as Time_Nome FROM Usuarios INNER JOIN Times ON Times.ID = Usuarios.Time WHERE Desafio_Vitorias > 0 ORDER BY Desafio_Vitorias DESC LIMIT $inicio,$limite");
...
<?php if ($pos == 1) { ?>
	<tr height="25" bgcolor="#B6B6B6">
		<td width="55">&nbsp;</td>
        <td width="205" style="padding-left: 31px" class="fonte1_negrito">Usuário</td>
		<td width="205" class="fonte1_negrito">Time</td>
		<td width="85" class="fonte1_negrito">Vitórias</td>
	</tr>
<?php } ?>
...
<?php } ?>
		...
		<td style="padding-top: 5px"><?=number_format($rs["Usuario_Desafio_Vitorias"],0,',','.')?></td>
	</tr>
...
if ($ranking_quantidade > $limite) {
	$p_nome = "ranking_desafios_vitorias";
